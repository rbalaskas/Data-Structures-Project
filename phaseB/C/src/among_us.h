...
int p;
int count;
struct Player* specific_player;
int distribute_given_tasks;
...
struct Task* minimumKey(struct Task *task);
...
void Player(struct Player* player);
...
void playerInorder(struct Player* player);

void taskInorder(struct Task* task);
...
void Inorder(struct Player* node,int thesi);
